...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if args.eval_first:
        ...
        # Save DUET predicted trajectories for visualization
        if default_gpu:
            os.makedirs(args.pred_dir, exist_ok=True)
            traj_save_path = os.path.join(args.pred_dir, f"trajectories_{env_name}.json")
            with open(traj_save_path, "w") as f:
                json.dump(preds, f)
            print(f"Saved trajectories to {traj_save_path}")
            if default_gpu:

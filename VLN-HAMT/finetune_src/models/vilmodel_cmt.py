        ...
        for batch_idx in range(align_imagine_embeds.size(0)):
            ...
            for imagine_idx in range(len(curr_sub_instr_imag_flag_bool)):
                ...
                for noun_phrase_idx in range(num_nounphrases_in_subinstr):
                    ...
                    max_txt_len = align_txt_embeds.size(1)
                    if curr_np_index[0] >= max_txt_len:
                        continue
                    valid_np_end = min(curr_np_index[1], max_txt_len - 1)
                    num_np_tokens_for_imagine += valid_np_end - curr_np_index[0] + 1
                    for token_in_np_idx in range(curr_np_index[0], valid_np_end + 1):
                        token_embedding = align_txt_embeds[batch_idx, token_in_np_idx]
                        all_nounphrase_embs_for_imagine.append(token_embedding)
                    assert all(txt_masks[batch_idx, curr_np_index[0]:valid_np_end+1]), 'Text_embeds is not valid where embedding addition is being applied.'

                if len(all_nounphrase_embs_for_imagine) > 0:

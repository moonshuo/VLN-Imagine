...
class VLNBertCMT(nn.Module):
    ...
    def forward(self, mode, txt_ids=None, txt_masks=None, txt_embeds=None, 
                ...
                align_txt_embeds=None, align_imagine_embeds=None, sub_instr_segs=None, sub_instr_imag_flag=None, noun_phrase_segs=None, obs_instr_ids = None, return_states=False, return_cross_attention_probs=False):
        ...
        if mode == 'language':
            ...
            if imagine_pano_img_feats is not None:
                imagine_pano_img_feats = self.drop_env(imagine_pano_img_feats)
            imagine_embeds = self.vln_bert(mode, imagine_pano_img_feats=imagine_pano_img_feats, imagine_masks=imagine_masks)
            return imagine_embeds #[8, 14, 768]

        if mode == 'align_with_contrastive_loss':
            contrastive_loss, aligned_imagine_embeds = self.vln_bert(mode, align_txt_embeds=align_txt_embeds, txt_masks=txt_masks, align_imagine_embeds=align_imagine_embeds, 
            ...
            hist_embeds = torch.stack(hist_embeds, 1) #from [8, 768] to [8, 1, 768].
            hist_masks = length2mask(hist_lens, size=hist_embeds.size(1)).logical_not() #[8,1]
            
            ob_img_feats = self.drop_env(ob_img_feats)
            
            if return_cross_attention_probs:
                act_logits, txt_embeds, hist_embeds, ob_embeds, cross_attn_probs, self_attn_scores_all_layers = self.vln_bert(
                    ...
                    ob_nav_types=ob_nav_types, ob_masks=ob_masks, imagine_embeds=imagine_embeds, imagine_masks=imagine_masks, return_cross_attention_probs=return_cross_attention_probs)
            else:
                act_logits, txt_embeds, hist_embeds, ob_embeds = self.vln_bert(
                    mode, txt_embeds=txt_embeds, txt_masks=txt_masks,
                    hist_embeds=hist_embeds, hist_masks=hist_masks,
                    ob_img_feats=ob_img_feats, ob_ang_feats=ob_ang_feats, 
                    ob_nav_types=ob_nav_types, ob_masks=ob_masks, imagine_embeds=imagine_embeds, imagine_masks=imagine_masks)
            # imagine_embeds: [8, 14, 768], imagine_masks: [8, 14]

            if not return_cross_attention_probs:
                if return_states:
                ...
                if return_states:
                    if self.args.no_lang_ca:
                        ...
                        states = txt_embeds[:, 0] * hist_embeds[:, 0]   # [CLS]
                    return act_logits, states, cross_attn_probs, self_attn_scores_all_layers
                return (act_logits, cross_attn_probs, self_attn_scores_all_layers)

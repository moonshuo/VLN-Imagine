...
from r2r.data_utils import ImageFeaturesDB, construct_instrs, ImaginationImageFeaturesDB
...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.aug is not None:
        aug_instr_data = construct_instrs(
            args.anno_dir, args.dataset, [args.aug], tokenizer=tok, max_instr_len=args.max_instr_len, aug_flag = True
        )
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if default_gpu:
        ...
        write_to_record_file(str(args) + '\n\n', record_file, verbose=False) #This line prints out all args on command line.
    ...
    if args.resume_file is not None:
        print("Resuming (inside Train) from loaded model...")
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        interval = min(args.log_every, args.iters-idx) #only works when start_iter = 0.
        iter = idx + interval
        ...
        if aug_env is None:
            ...
            for jdx in range(interval // 2):
                # Train with GT data
                print(f'\nStarting R2R training........')
                listner.args.imagine_enc_pano = True
                listner.vln_bert.vln_bert.config.imagine_enc_pano = True
                listner.env = train_env
                # args.ml_weight = 0.2
                listner.train(1, feedback=args.feedback)

                # Train with Augmented data
                print(f'Starting augmentation training........')
                listner.env = aug_env
                # args.ml_weight = 0.2
                listner.args.imagine_enc_pano = False
                listner.vln_bert.vln_bert.config.imagine_enc_pano = False
                listner.train(1, feedback=args.feedback)
        ...
        if default_gpu:
            ...
            write_to_record_file(
                ('%s (%d %d%%) %s' % (timeSince(start, float(iter)/(args.iters)), iter, float(iter)/(start_iter + args.iters)*100, loss_str)),
                record_file
            )
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    if args.resume_file is not None:
        print("Loaded the listener model (inside valid) at iter %d from %s" % (agent.load(args.resume_file), args.resume_file))
...
def main():
    ...
    if args.imagine_enc_pano:
        if args.imagination_data_v2:
            print('Using imaginations version v2.')
        if args.concat_imagine_with == 'visual':
            print('Starting obs+hist+imagine concat experiment...')
        print('Fix imagine embeds - ', args.fix_imagine_embeds)
    else:
        print('Starting baseline experiment...')

    if args.resume_file is not None:
        print(f'Resume checkpoint: {args.resume_file}')
    else:
